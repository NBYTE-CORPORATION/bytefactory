...
const BookMark = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (rightPanel && rightPanel.contains(event.target)) {
        ...
        const isNodeContainerClick = event.target.closest(
          ".node-outter-container"
        );
  ...
  const addNotepad = () => {
    ...
    if (newNotepad.parentId) {
      ...
      const updatedNodes = addNodeToFolder(
        nodes,
        newNotepad.parentId,
        newNotepad
      );
  ...
  const addNodeToFolder = (nodes, parentId, newNode) => {
    return nodes.map((node) => {
      if (node.id === parentId) {
        ...
        return {
          ...node,
          children: addNodeToFolder(node.children, parentId, newNode),
        };
      }
      return node;
    });
  ...
  const handleDeleteNode = () => {
    ...
    if (selectedPath.includes(selectedNodeId)) {
      const newPath = selectedPath.slice(
        0,
        selectedPath.indexOf(selectedNodeId)
      );
  ...
  const renderContextMenu = () => {
    ...
    return (
      ...
      >
        <li onClick={() => handleStartEditing(contextMenu.selectedNodeId)}>
          이름 변경
        </li>
  ...
  const renderColumnView = () => {
    if (nodes.length === 0) {
      return <div className="empty-message">북마크를 등록해주세요.</div>;
    }
    ...
    columns.push(
      <div
        key="column-root"
        style={{ minWidth: "200px", padding: "10px 0px 10px 10px" }}
      >
        {renderNodes(
  ...
  const renderNodes = (nodes) => {
    ...
    return (
      <div
        className="node-outter-container"
        style={{
          minWidth: "200px",
        ...
        {nodes.map((node) => (
          ...
          >
            <div
              ...
              style={{
                backgroundColor: selectedPath.includes(node.id)
                  ? "#007bff"
                  : "transparent",
            ...
            >
              {editingNodeId === node.id ? (
                ...
                <div className="div-node-label">
                    ...
                    {node.type === "folder" ? (
                      ...
                      >
                        <img
                          ...
                          onLoad={(e) => {
                            e.target.style.display = "block"; // 이미지를 보여줍니다.
                            e.target.nextSibling.style.display = "none"; // 이미지가 로드되면 🔗을 숨깁니다.
                          }}
                          onError={(e) => {
                            e.target.onerror = null;
                            e.target.parentNode.innerText = "🔗"; // 이미지가 없을 때 표시할 텍스트를 설정합니다.
                          }}
                        />
                        <span
                          style={{
                  ...
                  <span className="node-label node-label2">{node.name}</span>
                </div>
              )}
            </div>
          </div>
    ...
    while (stack.length > 0) {
      ...
      if (
        ...
        currentNode.children.some(
          (child) => Number(child.id) === Number(nodeId)
        )
      ) {
        return currentNode;
      }
  ...
  const handleDrop = (e, targetNodeId) => {
    ...
    if (draggedNodeId && Number(draggedNodeId) !== Number(targetNodeId)) {
      ...
      updatedNodes = updatedNodes.filter(
        (node) => Number(node.id) !== Number(draggedNodeId)
      );
  ...
  const isFindNodeChildren = (nodes, nodeId, targetNodeId) => {
    ...
    if (draggedNode.children) {
      for (const child of draggedNode.children) {
        ...
        if (
          Number(child.id) === Number(targetNodeId) ||
          isFindNodeChildren(nodes, child.id, targetNodeId)
        ) {
          return true;
        }
      }
  ...
  const moveNode = (nodes, draggedNodeId, targetNodeId) => {
    ...
    if (
      ...
      targetNode.children.some(
        (child) => Number(child.id) === Number(draggedNodeId)
      )
    ) {
      return nodes;
    }
    ...
    if (draggedParentNode) {
      draggedParentNode.children = draggedParentNode.children.filter(
        (child) => Number(child.id) !== Number(draggedNodeId)
      );
    }
    ...
    if (targetParentNode) {
      const updatedChildren = targetParentNode.children
        .map((child) => {
  ...
  const moveNode2 = (nodes, draggedNodeId, targetNodeId, isTopHalf) => {
    ...
    const nodeToAdd = {
      ...draggedNode,
      parentId: targetParentNode ? targetParentNode.id : null,
    };
    ...
    if (isTopHalf) {
      ...
      if (targetParentNode) {
        const index = targetParentNode.children.findIndex(
          (child) => Number(child.id) === Number(targetNodeId)
        );
        targetParentNode.children.splice(index, 0, nodeToAdd);
      } else {
        // 타겟 노드가 최상위 노드인 경우
        const index = updatedNodes.findIndex(
          (node) => Number(node.id) === Number(targetNodeId)
        );
      ...
      <div className="left-panel">
        ...
        <div>
          ...
          <input
            ...
            placeholder="북마크 URL"
          />
          <button onClick={addBookmark} className="button">
            북마크 추가
          </button>
          <input
            type="text"
            ...
            placeholder="메모 이름"
          />
          <button onClick={addNotepad} className="button">
            메모 추가
          </button>
          <button onClick={openFinder} className="btn_finder">
            <p>
              파일탐색기 / <img src={FinderIcon} alt="Finder" />
            </p>
          </button>
        </div>
      </div>
